...
#         if num > 0 and self.is_beautiful(count): nums.append(num)
...
class Solution:
    def generate(self, n: int, current: int, remaining: int, count: list[int]) -> int:
        if remaining == 0:
            if current > n and all(c == 0 or c == i for i, c in enumerate(count)): return current
            return 0
        result = 0
        for d in range(1, 10):
    ...
    def nextBeautifulNumber(self, n: int) -> int:
        ...
        if result == 0: result = next_len_result
        return result