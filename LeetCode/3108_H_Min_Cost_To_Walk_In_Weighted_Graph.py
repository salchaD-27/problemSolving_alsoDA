class Solution:
    def minimumCost(self, n, edges, queries):
        ...
        for query in queries:
            start, end = query
            if self._find(start) != self._find(end): answer.append(-1)
            else:
    ...
    def _find(self, node):
        if self.parent[node] == -1: return node
    ...
    def _union(self, node1, node2):
        ...
        if self.depth[root1] < self.depth[root2]: root1, root2 = root2, root1
        self.parent[root2] = root1
        if self.depth[root1] == self.depth[root2]: self.depth[root1] += 1